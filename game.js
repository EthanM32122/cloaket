...
function defaultData(){return{tokens:CATALOG.startTokens||1500,exp:0,packsOpened:0,mined:0,messages:0,role:'player',inventory:{},friends:[],clan:'',lastMine:0,lastClaim:0,starter:false}}
...
function rollPack(packName,luckMultiplier=1){
  ...
  let weights = entries.map(b=>{
    let c = b.chance || 1;
    if(luckMultiplier>1 && (b.rarity==='Legendary'||b.rarity==='Chroma'||b.rarity==='Mystical')) c*=luckMultiplier;
    return c;
  });
...
function renderCraft(){
  ...
  } else res.innerHTML='<div class="big">🔨</div>Add blooks to see what you could craft.';
  renderCraftInv();
}
...
function doMine(){
  const u=currentUser();const d=getData(u);
  const now=Date.now();
  if(now-(d.lastMine||0)<3000){document.getElementById('mineResult').textContent='Cooldown...';return}
  const gain=5+Math.floor(Math.random()*20);
  d.tokens+=gain;d.mined=(d.mined||0)+1;d.lastMine=now;d.exp+=1;
  saveData(u,d);
  document.getElementById('mineResult').textContent='+'+gain+' tokens';
  refreshUI();
}
function claimDaily(){
  const u=currentUser();const d=getData(u);
...
function renderClans(){
  ...
  const q=(document.getElementById('clanSearch')?.value||'').toLowerCase();
  let clans=getGlobal('clans',[]);
  if(q)clans=clans.filter(c=>c.name.toLowerCase().includes(q)||c.owner.toLowerCase().includes(q));
  const d=getData(currentUser());
  if(!clans.length){list.innerHTML='<div class="empty">No clans yet. Create one!</div>';return}
  list.innerHTML='';
  clans.forEach(c=>{
    const row=document.createElement('div');row.className='lb-row';
    row.innerHTML='<div style="flex:1"><b>'+c.name+'</b><div class="muted" style="font-size:12px">Owner '+c.owner+' · '+(c.members||[]).length+' members</div></div>';
    const btn=document.createElement('button');btn.className='btn btn-primary btn-sm';
    if(d.clan===c.name){btn.textContent='Leave';btn.onclick=()=>{d.clan='';saveData(currentUser(),d);c.members=(c.members||[]).filter(m=>m!==currentUser());setGlobal('clans',getGlobal('clans',[]).map(x=>x.name===c.name?c:x));refreshUI()}}
    else{btn.textContent='Join';btn.onclick=()=>{d.clan=c.name;saveData(currentUser(),d);c.members=c.members||[];if(!c.members.includes(currentUser()))c.members.push(currentUser());const all=getGlobal('clans',[]).map(x=>x.name===c.name?c:x);setGlobal('clans',all);refreshUI()}}
...
function addFriend(){
  ...
  const d=getData(currentUser());d.friends=d.friends||[];
...
function renderChat(){
  ...
  msgs.slice(-100).forEach(m=>{
    ...
    div.innerHTML='<span class="who">'+m.user+'</span><span class="txt">'+escapeHtml(m.text)+'</span>';
    box.appendChild(div);
  });
...
function setLbMode(m){lbMode=m;document.getElementById('lbXp').classList.toggle('active',m==='xp');document.getElementById('lbMine').classList.toggle('active',m==='mine');renderLB()}
function renderLB(){
  const list=document.getElementById('lbList');if(!list)return;
  const users=Object.keys(getUsers());
  const rows=users.map(u=>{const d=getData(u);return{u,exp:d.exp||0,mined:d.mined||0,tokens:d.tokens||0}});
  rows.sort((a,b)=>lbMode==='mine'?b.mined-a.mined:b.exp-a.exp);
  list.innerHTML='';
  rows.forEach((r,i)=>{
    const row=document.createElement('div');row.className='lb-row';
    row.innerHTML='<div class="rank">#'+(i+1)+'</div><div style="flex:1"><b>'+r.u+'</b><div class="muted" style="font-size:12px">'+(lbMode==='mine'?r.mined+' mines':r.exp+' XP')+' · '+r.tokens.toLocaleString()+' tokens</div></div>';
    list.appendChild(row);
  });
}
function renderGifts(){
  const el=document.getElementById('giftList');if(!el)return;
  const gifts=getGlobal('gifts',[]).filter(g=>g.to===currentUser()||g.from===currentUser());
  if(!gifts.length){el.innerHTML='No gifts.';return}
  el.innerHTML='';
  gifts.forEach(g=>{
    const row=document.createElement('div');row.className='lb-row';
    row.innerHTML='<div style="flex:1">'+g.from+' → '+g.to+': '+g.blook+'</div>';
    if(g.to===currentUser()&&!g.claimed){
      const btn=document.createElement('button');btn.className='btn btn-primary btn-sm';btn.textContent='Claim';
      btn.onclick=()=>{const d=getData(currentUser());d.inventory[g.blook]=(d.inventory[g.blook]||0)+1;saveData(currentUser(),d);g.claimed=true;setGlobal('gifts',getGlobal('gifts',[]));refreshUI()};
      row.appendChild(btn);
    }
    el.appendChild(row);
  });
...
function sendGift(){
  const blook=document.getElementById('giftBlook').value;
  const to=(document.getElementById('giftUser').value||'').trim();
  const d=getData(currentUser());
  if(!blook||!to)return;
  if((d.inventory[blook]||0)<1){alert('You do not own that');return}
  d.inventory[blook]--;saveData(currentUser(),d);
  const gifts=getGlobal('gifts',[]);gifts.push({from:currentUser(),to,blook,claimed:false,ts:Date.now()});setGlobal('gifts',gifts);refreshUI();
}
function fillSelects(){
  const d=getData(currentUser());
  const owned=Object.entries(d.inventory).filter(([,q])=>q>0).map(([n])=>n);
  ['listBlook','giftBlook'].forEach(id=>{
    const sel=document.getElementById(id);if(!sel)return;
    const cur=sel.value;sel.innerHTML=owned.length?owned.map(n=>'<option>'+n+'</option>').join(''):'<option value="">No blooks</option>';
    if(owned.includes(cur))sel.value=cur;
  });
}
function changeUsername(){
  const neu=(document.getElementById('setUser').value||'').trim();
  if(neu.length<3||neu.length>20){alert('3-20 chars');return}
  const users=getUsers();if(users[neu]){alert('Taken');return}
  const old=currentUser();users[neu]=users[old];delete users[old];
  safeSet('ck_users',JSON.stringify(users));
  const data=safeGet('ck_data_'+old,'{}');safeSet('ck_data_'+neu,data);safeSet('ck_current',neu);
  refreshUI();alert('Username updated');
}
function changePassword(){
  const p=document.getElementById('setPass').value||'';
  if(p.length<8){alert('Min 8 chars');return}
...
function deleteAccount(){
  ...
  const u=currentUser();const users=getUsers();delete users[u];safeSet('ck_users',JSON.stringify(users));safeSet('ck_current','');logout();
}
function adminGive(){
  const t=(document.getElementById('admUser').value||'').trim();
  const amt=parseInt(document.getElementById('admAmt').value,10)||0;
  if(!getUsers()[t]){alert('No user');return}
  const d=getData(t);d.tokens+=amt;saveData(t,d);alert('Gave '+amt+' to '+t);refreshUI();
}
function adminRole(){
  const t=(document.getElementById('admUser').value||'').trim();
  const role=document.getElementById('admRole').value;
  if(!getUsers()[t]){alert('No user');return}
  const d=getData(t);d.role=role;saveData(t,d);alert(t+' is now '+role);buildNav();
}
function buyStarter(){
  const u=currentUser();const d=getData(u);
  if(d.starter){alert('Already owned');return}
  if(!confirm('Demo purchase: grant Starter Bundle for free?'))return;
  d.starter=true;d.tokens+=75000;saveData(u,d);alert('Starter Bundle unlocked!');refreshUI();
}
function viewPlayerStats(){
  ...
  alert(name+'\nTokens: '+d.tokens+'\nXP: '+d.exp+'\nLevel: '+levelForExp(d.exp)+'\nBlooks: '+Object.keys(d.inventory||{}).length);
}
(function(){const u=currentUser();if(u&&getUsers()[u])afterLogin(u);})();
